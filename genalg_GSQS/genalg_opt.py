...
def genalg_loop(starting_atoms_in,population_size=4,ngenerations=10):
    ...
    while igen <= ngenerations and best_eval > convthr and not conv_flag:
        ...
        for ii in range(0,population_size,2):
            ...
            for c in cs:
                ...
                if rndmut <= r_mut:
                    ...
                    muttyp = mutation_type_from_prob(choice_probs = {'perturb_one': 0.0, 'perturb_N' : 0.0, 'flip_one' : 0.5, 'flip_N' : 0.5})
        ...
        population = children
        scores = [get_cost(test_atoms,ii,igen) for ii,test_atoms in enumerate(population)]
        #print (igen, np.round(np.var(best_evals[int(ngenerations/conv_check)-int(ngenerations/10):]),14) )
