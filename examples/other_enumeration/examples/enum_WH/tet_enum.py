...
a=3.19
base_species=[['W'],['W']]
primitive = bulk(base_species[0][0] , 'bcc', a=a,cubic=True)
print(primitive)
print(primitive.get_scaled_positions())
import itertools

locs = [[0.25,0.75],[0.25,0.75],[0.25,0.75]]

possible_tets = []
possible_tets += sorted(list(set([p for p in itertools.permutations([0.0,0.5,0.25])])))
possible_tets += sorted(list(set([p for p in itertools.permutations([0.0,0.5,0.75])])))

possible_tets = [np.array(pt) for pt in possible_tets]

for tet in possible_tets:
    primitive.append(Atom('H',(tet*a).tolist()))
    base_species.append(['H','X'])
write('conv.cif',primitive)
enumerated = enumerate_structures(primitive, range(1,1+1), base_species)
...
for icrystal,crystal in enumerate(enumerated):
    ...
    write('tet_%04d_vac.cif'%icrystal,crystal)
    write('tet_%04d.cif'%icrystal,atoms)
