...
cross_weight=-0.001 #closeness to target as a joint distribution min/max just flip this sign (pos being minimize).
...
em=GSQSModel(1,n_descs,mask=mask)
# use internal structure generator to build candidates
g=internal_generate_cell(0,desired_size=vnp.random.choice(range(mincellsize,maxcellsize)),template=None,desired_comps=target_comps,use_template=None,min_typ=min_typ_global)
print ('model',em)
...
while i <= n_totconfig:
    ...
    if not randomize_comps:
        g = internal_generate_cell(i,desired_size=vnp.random.choice(range(mincellsize,maxcellsize)),template=None,desired_comps=target_comps,use_template=None,min_typ=min_typ_global)
    else:
        target_comps_rnd = rand_comp(target_comps)
        g = internal_generate_cell(i,desired_size=vnp.random.choice(range(mincellsize,maxcellsize)),template=None,desired_comps=target_comps_rnd,use_template=None,min_typ=min_typ_global)
    sampler=GSQSSampler(em,g)
