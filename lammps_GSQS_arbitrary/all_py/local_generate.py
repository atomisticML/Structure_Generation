...
def gen_random(idi,N=32,desired_comps={'Ni':1.0}):
	...
	cell_mult = np.eye(3)*2  #hnf_mats[cell_id]

	#tmpats = bulk('Ni','bcc',a=a,cubic=False)
    tmpats = bulk(chems[0],cubic=True)
	...
	write('atoms_%d.data' %idi,these_atoms,format='lammps-data')
	return 'read_data    atoms_%d.data' %idi
